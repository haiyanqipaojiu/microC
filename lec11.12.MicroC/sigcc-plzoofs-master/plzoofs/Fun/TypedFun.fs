    ...
    | If (e1, e2, e3) ->
        ...
        | TypB ->
            ...
            if t2 = t3 then
                ...
                failwith "If: branch types differ"
...
let ex2 =
    Letfun(
        ...
        TypI,
        Let("n", CstI 5, Call(Var "fac", Var "n"))
    )

let fac5 = eval ex2 []

let ex3 =
    Let("b", Prim("=", CstI 1, CstI 2), If(Var "b", CstI 3, CstI 4))

let ex4 =
    Let("b", Prim("=", CstI 1, CstI 2), If(Var "b", Var "b", CstB false))

let ex5 =
    If(Prim("=", CstI 11, CstI 12), CstI 111, CstI 666)

let ex6 =
    Letfun("inf", "x", TypI, Call(Var "inf", Var "x"), TypI, Call(Var "inf", CstI 0))

let types =
    List.map typeCheck [ ex1; ex2; ex3; ex4; ex5; ex6 ]

(* Examples of type errors; should throw exception when run: *)

let exErr1 =
    Let("b", Prim("=", CstI 1, CstI 2), If(Var "b", Var "b", CstI 6))
