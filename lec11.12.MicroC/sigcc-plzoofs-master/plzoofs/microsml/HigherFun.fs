...
and closure = Closure of string * string * expr<typ> * value env (* (f, x, fBody, fDeclEnv) *)
...
let rec evalExpr (env: value env) (e: expr<typ>) (cont: value -> answer) (econt: value -> answer) : answer =
    match e with
    | CstI (i, _) -> cont (Int i)
        ...
        evalExpr
            ...
            (fun v1 ->
                ...
                | ("print", TypI) ->
                    ...
                    cont v1
                | ("print", TypB) ->
                    printf "%s " (if v1 = Int 0 then "false" else "true")
                    cont v1
                | ("print", TypL _) ->
                    printf "%s " (ppValue v1)
                    cont v1
                | ("print", _) ->
                    printf "%s " (ppValue v1) (* Print for all types *)
                    cont v1
                | ("hd", _) ->
                    match v1 with
                ...
                | _ -> Abort("Prim1 " + ope + " not implemented"))
            econt
    | Prim2 (ope, e1, e2, _) ->
        evalExpr
            env
            ...
            (fun v1 ->
                evalExpr
                    ...
                    (fun v2 ->
                        ...
                        | _ -> Abort("unknown primitive " + ope + " or wrong type"))
                    econt)
            econt
    | AndAlso (e1, e2, _) ->
        evalExpr
    ...
    | OrElse (e1, e2, _) ->
        evalExpr
            ...
            (fun v1 ->
                ...
                | _ -> Abort "evalExpr OrElse")
            econt
    | Seq (e1, e2, _) ->
        evalExpr env e1 (* Disregard result of e1 and return e2 *) (fun _ -> evalExpr env e2 cont econt) econt
    ...
    | Fun (x, fBody, _) ->
        let freeVars =
            Set.toList (freevars fBody - (set [ x ]))

        let freeVarsEnv =
            List.filter (fun (f, _) -> List.exists ((=) f) freeVars) env
                     ...
                     | Closure (f, x, fBody, fDeclEnv) ->
                         evalExpr
                             ...
                             (fun xVal ->
                                 ...
                                 evalExpr fBodyEnv fBody cont econt))
                        econt
                | _ -> Abort "evalExpr Call: not a function")
            econt
...
and evalValdecs
    ...
    | Fundecs fs :: ts' ->
        let closureRefs =
            List.map (fun (f, x, fBody) -> (f, ref (Closure(f, x, fBody, env)))) fs

        let updClosRef (f, closRef) =
            (match !closRef with
             | Closure (f, x, fBody, _) ->
                 let freeVars =
                     Set.toList (freevars fBody - (set [ x; f ]))

                 let freeVarsEnv =
                     List.filter (fun (f, _) -> List.exists ((=) f) freeVars) env

                 let freeClosureRefs =
                     List.filter (fun (f, _) -> List.exists ((=) f) freeVars) closureRefs

                 let fBodyEnv =
                     (List.map (fun (f, closRef) -> (f, ClosureRef closRef)) freeClosureRefs)
                     @ freeVarsEnv
        ...
        let env' =
            (List.map (fun (f, closRef) -> (f, ClosureRef closRef)) closureRefs)
            @ env
...
and evalProg (env: value env) (t: program<typ>) : answer =
    match t with
    | Prog (valdecs, e) ->
        evalValdecs env valdecs e (fun v -> Result v) (fun v -> Abort("Uncaught exception: " + (ppValue v)))
...
let check e =
    let rec check' (e: expr<'a>) (env: string list) : bool =
        ...
        | If (e1, e2, e3) -> check' e1 env && check' e2 env && check' e3 env
    ...
    and checkValdec' (b: bool, env: string list) (valdec: valdec<'a>) : bool * string list =
        ...
        | Fundecs fs ->
            let fEnv =
                List.foldBack (fun (f, _, _) env -> f :: env) fs env (* fBody may recursively call f *)

            (List.foldBack
                (fun (_, x, fBody) acc -> check' fBody (x :: fEnv) && acc)
                fs
                b (* Only include x for function f *) ,
             fEnv)
