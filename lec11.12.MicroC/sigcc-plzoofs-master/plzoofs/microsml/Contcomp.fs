...
let (resetFuncs, addFunc, getFuncs, resetGlobalInit, addGlobalInit, getGlobalInit) =
    ...
    let get iss =
        fun () -> (List.concat << List.rev) !iss

    (reset funcInsts, add funcInsts, get funcInsts, reset globalInsts, add globalInsts, get globalInsts)
...
let rec lookup env x =
    ...
    | [] ->
        if x = exnNumVar then
            snd (List.head (fst exnNumVarEnv)) (* TODO eliminate need *)
        else
...
let rec addINCSP m1 C : instr list =
    ...
    | (true, Label lab :: RET m2 :: _) ->
        RET(m2 - m1)
        :: C (* Becomes: RET(m2-m1)::Label lab::RET m2 *)
...
let loadVar varEnv x C : instr list =
    ...
    | Closvar offset ->
        ...
        :: LDI
           :: HEAPLDI offset
              :: C (* First access closure and then offset into closure *)
...
let rec cExpr (kind: int -> var) (varEnv: varEnv) (e: expr<typ>) (C: instr list) : instr list =
    ...
    | Prim1 (ope, e1, _) ->
        cExpr
            ...
            (match (ope, getTypExpr e1) with
             ...
             | ("print", t) ->
                 debug (
                     "Warning: cExpr.Prim1: print not implemented on type "
                     + (TypeInference.showType t)
                 )
    ...
    | AndAlso (e1, e2, _) ->
        match C with
        | IFZERO lab :: _ -> cExpr kind varEnv e1 (IFZERO lab :: cExpr kind varEnv e2 C)
        | IFNZRO labthen :: C1 ->
    ...
    | OrElse (e1, e2, _) ->
        match C with
        | IFNZRO lab :: _ -> cExpr kind varEnv e1 (IFNZRO lab :: cExpr kind varEnv e2 C)
        | IFZERO labthen :: C1 ->
            let (labelse, C2) = addLabel C1
            ...
            cExpr
                ...
                e1
                (IFNZRO labelse
                 :: cExpr kind varEnv e2 (IFZERO labthen :: C2))
        | _ ->
            let (jumpend, C1) = makeJump C
            ...
            cExpr
                ...
                (IFNZRO labtrue
                 :: cExpr kind varEnv e2 (addJump jumpend C2))
    | Seq (e1, e2, _) -> cExpr kind varEnv e1 (addINCSP (* Remove result of e1 *) -1 (cExpr kind varEnv e2 C))
    | Let (valdecs, letBody) ->
        let ((_, fdepth') as bodyEnv, vdEnvs) =
            List.fold
                (fun (accEnv, accVdEnv) vd ->
                    let (nextEnv, vdEnv) = genValdecEnv kind accEnv vd
                    (nextEnv, (vd, vdEnv) :: accVdEnv))
                (varEnv, [])
                valdecs
        ...
        let addrFirstVal C =
            GETSP :: addCST (numVals - 1) (SUB :: C)

        let iBody C =
            cExpr kind (incVarIdx bodyEnv 1) letBody C (* Make room for addrFirstVal *)
    ...
    | Fun (x, fBody, _) ->
        ...
        let _ =
            addFunc funcLab (cExpr Locvar (bodyEnv, 2) fBody [ RET 2 ])

        let sizeClos = List.length fvsClos + 1

        let codeFreevars' =
            List.foldBack (loadVar varEnv) fvsClos (ACLOS sizeClos :: HEAPSTI sizeClos :: C)

        PUSHLAB funcLab :: codeFreevars'
    | Call (eFun, eArg, tOpt, _) ->
        let cInst C =
            match (!opt_p, tOpt) with
            | (true, Some true) -> TCLOSCALL 1 :: deadcode C
            | _ -> CLOSCALL 1 :: C
...
and genValdecEnv (kind: int -> var) ((env, fdepth) as curEnv) vd =
    ...
    | Fundecs fs ->
        let newEnv =
            List.fold (fun (env, fdepth) (f, x, fBody) -> ((f, kind fdepth) :: env, fdepth + 1)) curEnv fs
        ...
        let fsfvs =
            List.map
                (fun (f, x, fBody) ->
                    ...
                    (f, x, fBody, fvsGlobalInScope, fvsClos, List.length fvsClos + 1, labFunc))
                fs
        (* Code to allocate closures *)
        let iaClos C =
            List.foldBack (fun (_, _, _, _, _, sizeClos, _) C -> ACLOS sizeClos :: C) fsfvs C
        (* Code to copy free variables and code label to each closure *)
        let iFillClos C =
            List.foldBack
                (fun (f, _, _, _, fvsClos, sizeClos, funcLab) C ->
                    let codefvsClos C =
                        List.foldBack (loadVar varEnv) fvsClos C

                    let codeClosPtr C = loadVar varEnv f C

                    PUSHLAB funcLab
                    :: codefvsClos (codeClosPtr (HEAPSTI sizeClos :: addINCSP -1 C)))
                fsfvs
                C
...
and cProgram (p: program<typ>) : instr list =
    ...
    let initEnv =
        exnNumVarEnv (* Global exception number as first global variable, addr 0 *)

    let _ =
        addGlobalInit (newLabelWName "G_ExnVar") (initExnNumCode 0)
    ...
    | Prog (valdecs, e) ->
        let ((bodyEnv, fdepth'), vdEnvs) =
            List.fold
                (fun (accEnv, accVdEnv) vd ->
                    let (nextEnv, vdEnv) = genValdecEnv Glovar accEnv vd
                    (nextEnv, (vd, vdEnv) :: accVdEnv))
                (initEnv, [])
                valdecs
        ...
        let numVals =
            fdepth' (* Remove everything below the end result value. *)

        let iVals C =
            List.foldBack (cValdec Glovar) vdEnvs' C

        let _ =
            addGlobalInit (newLabelWName "G_Valdecs") (iVals [])

        let _ =
            addFunc labMain ((cExpr Locvar (bodyEnv, 0) e) [ RET 0 ])
        ...
        @ (GETSP
           :: addCST
               ...
               (SUB
                :: CALL(0, labMain)
                   :: STI :: addINCSP -numVals (STOP :: getFuncs ())))
...
let compileToFile (opt_p', debug_p', verbose_p', program, fname) =
    let _ =
        (debug_p := debug_p'
         opt_p := opt_p'
         verbose_p := verbose_p') (* Set compiler flags *)
