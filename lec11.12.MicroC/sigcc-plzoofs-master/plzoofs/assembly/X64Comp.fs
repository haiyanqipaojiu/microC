(* File Assembly/X86Comp.sml
    ...
    * Uses X64.fs for code emission instead of Machine.fs.
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) (tr : reg64) (pres : reg64 list) : x86 list = 
    ...
    | Prim1(ope, e1) ->
        ...
        @ (match ope with
           ...
           | "printi" -> [Ins1("push", Reg tr); PRINTI; Ins("add rsp, 8")]
           | "printc" -> [Ins1("push", Reg tr); PRINTC; Ins("add rsp, 8")]
...
let compileToFile program fname = 
    ...
    let code = [X64.stdheader;
                X64.beforeinit argc]
               @ X64.code2x86asm globalinit
               @ [X64.pushargs]
               @ X64.code2x86asm maincall
               @ [X64.popargs]
               @ X64.code2x86asm functions
