...
let allocate (kind: int -> var) (typ, x) (varEnv: varEnv) : varEnv * x86 list =
    ...
    | TypA (t, Some i) ->
        let newEnv =
            ((x, (kind (fdepth + i), typ)) :: env, fdepth + i + 1)
...
let makeGlobalEnvs (topdecs: topdec list) : varEnv * funEnv * x86 list =
    let rec addv decs varEnv funEnv =
        ...
        | dec :: decr ->
            ...
            | Fundec (tyOpt, f, xs, body) -> addv decr varEnv ((f, ("_" + f, tyOpt, xs)) :: funEnv)
...
let rec cStmt stmt (varEnv: varEnv) (funEnv: funEnv) : x86 list =
    ...
    | While (e, body) ->
        ...
        [ Jump("jmp", labtest); Label labbegin ]
        @ cStmt body varEnv funEnv
          @ [ Label labtest ]
            @ cExpr e varEnv funEnv Ebx []
              @ [ Ins2("cmp", Reg Ebx, Cst 0)
                  Jump("jnz", labbegin) ]
    | Expr e -> cExpr e varEnv funEnv Ebx []
    | Block stmts ->
        ...
        code
        @ [ Ins2("sub", Reg Esp, Cst(4 * (snd varEnv - fdepthend))) ]
    | Return None ->
...
and cExpr (e: expr) (varEnv: varEnv) (funEnv: funEnv) (tr: reg32) (pres: reg32 list) : x86 list =
    ...
    | Prim2 (ope, e1, e2) ->
        ...
        @ let tr' = getTempFor (avoid @ pres)
          in
          cExpr e2 varEnv funEnv tr' (tr :: pres)
          @ match ope with
            | "+" -> [ Ins2("add", Reg tr, Reg tr') ]
            | "-" -> [ Ins2("sub", Reg tr, Reg tr') ]
            | "*" ->
                [ Ins2("mov", Reg Eax, Reg tr) ]
                @ preserve Edx (tr :: pres) [ Ins1("imul", Reg tr') ] // Invalidates EDX
                  @ [ Ins2("mov", Reg tr, Reg Eax) ]
            | "/" ->
                [ Ins2("mov", Reg Eax, Reg tr) ]
                @ preserve
    ...
    | Andalso (e1, e2) ->
        ...
        @ [ Ins2("cmp", Reg tr, Cst 0)
            Jump("jz", labend) ]
          @ cExpr e2 varEnv funEnv tr pres @ [ Label labend ]
    | Orelse (e1, e2) ->
        let labend = newLabel ()
...
and cAccess access varEnv funEnv (tr: reg32) (pres: reg32 list) : x86 list =
    ...
    | AccIndex (acc, idx) ->
        ...
        @ [ Ins2("mov", Reg tr, Ind tr) ]
          @ let tr' = getTempFor (tr :: pres)
            in
            cExpr idx varEnv funEnv tr' (tr :: pres)
...
let compileToFile program fname =
    ...
    let code =
        [ X86.stdheader; X86.beforeinit argc ]
        @ X86.code2x86asm globalinit
          @ [ X86.pushargs ]
            @ X86.code2x86asm maincall
              @ [ X86.popargs ] @ X86.code2x86asm functions
